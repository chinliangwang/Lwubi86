-- lua/lunarDate.lua
-- Minimal lunar date utilities (Date2LunarInfo, Date2LunarDate)
-- Portions adapted from rime-wubi's lua/lunarDate.lua
-- Original author: 空山明月 (see https://github.com/rime/rime-wubi)
-- License: preserve original license/attribution when applicable

-- <helper functions copied/adapted from original implementation>
-- 十进制转二进制
...
-- 公历转农历（基于社区实现）
-- 返回值：Formatted string (e.g., "乙巳年(蛇)闰四月初三")
-- 另提供 Date2LunarInfo 返回结构化信息
function Date2LunarInfo(Gregorian)
    -- constants
    ...
    local wNongliData = { -- truncated: full table copied from community implementation
        "AB500D2","4BD0883","4AE00DB","A5700D0","54D0581","D2600D8","D9500CC","655147D","56A00D5","9AD00CA","55D027A","4AE00D2",
    ...
    if (Year>2100 or Year<1899 or Month>12 or Month<1 or Day<1 or string.len(Gregorian)<8) then
        return nil, "无效日期"
    end
    ...
    local Date3 = diffDate(Date1,Date2)
    if (Date3<0) then
        tb1=Analyze(Data0)
        Year=Year-1
        MonthInfo=tb1[1]
    ...
    local thisMonth,thisDays,LDay,Isleap,LunarYear,LunarMonth,LMonthNum
    for i=1,13 do
    ...
    local LunarMonthName
    if Isleap>0 then LunarMonthName = "闰" .. (cMonName[LMonth] or "") else LunarMonthName = cMonName[LMonth] end

    local LunarYearStr = cTianGan[math.fmod(LYear-4,10)+1] .. cDiZhi[math.fmod(LYear-4,12)+1] .. "年" -- keep year string
    local LunarDayName = cDayName[LDay]

    local out = {
        year = LunarYearStr, -- e.g., "乙巳年"
        zodiac = cShuXiang[math.fmod(LYear-4,12)+1],
        monthName = LunarMonthName, -- e.g., "闰四月" or "四月"
        dayName = LunarDayName, -- e.g., "初三"
        monthNum = LMonthNum,
        dayNum = LDay,
        isLeap = (Isleap==1)
    }
...
function Date2LunarDate(Gregorian)
    local info, err = Date2LunarInfo(Gregorian)
    if not info then return err or "无效日期" end
    -- Format: 年 + 月 + 日 (omit zodiac by default, user wants like: "乙巳年 闰四月 初三")
    return info.year .. " " .. info.monthName .. " " .. info.dayName
end

-- Export functions
return {
    Date2LunarInfo = Date2LunarInfo,
